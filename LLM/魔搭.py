...
class LLM:
    ...
    def causal_discovery(self):
        ...
        messages = [{"role": "system", "content": "你是用于因果发现的人工智能助手."},
                    ...
                    {"role": "user", "content": "根据以上数据返回一个因果发现矩阵(表示一个有向无环图).除此之外不要返回任何东西,也不需要注释."}]
...
if __name__ == '__main__':
    model = LLM()
    model("dataSet/lucas.csv")
    model.causal_discovery()
    print(model.answer)
    B_est = model.get_causal_matrix()
    display_graph(B_est,model.labels,model.path)   
    B_true = pd.read_csv("dataSet/lucas_graph.csv").values
    acc = calculate_accuracy(B_true,B_est)
    print(acc)
