...
def parse_args():
    ...
    parser.add_argument('--lambda1', type=float, default=0.001, help='L1 regularization parameter')
    parser.add_argument('--lambda2', type=float, default=0.001, help='L2 regularization parameter')
...
if __name__ == '__main__':
    ...
    labels = np.genfromtxt(args.X_path, delimiter=',', max_rows=1, dtype=str)
    X = np.loadtxt(args.X_path, delimiter=',', skiprows=1)
    n, d = X.shape
    model = NotearsMLP(dims=[d, args.hidden, 1], bias=True)
    W_est = notears_nonlinear(model, X, lambda1=args.lambda1, lambda2=args.lambda2)
    assert is_dag(W_est)
    np.savetxt(args.W_path, W_est, delimiter=',', fmt='%.3f')

    path = current_dir+'//'+args.X_path.split('/')[-1].split('.')[0]+'_est_graph'
    display_graph(W_est,labels,path)
